...
export default function App() {
  const raceStart = new Date(`${RACE_START_DATE}T${RACE_START_TIME}`);
  ...
  const [times, setTimes] = useState(() => {
    ...
    // Default: start location has pre-set times
    return { 1: { arrival: `${RACE_START_DATE}T${RACE_START_TIME}`, departure: `${RACE_START_DATE}T${RACE_START_TIME}` } };
  });

  const [editing, setEditing] = useState(null); // locationId currently being edited
  ...
  // Find last location with a departure time
  let lastDepartureMile = 0;
  let lastDepartureTime = raceStart;
  let legPaces = []; // mph per leg
  ...
  const derived = locations.map((loc, idx) => {
    ...
    if (arrival && idx > 0) {
      ...
      const hrs = hoursBetween(prevDep, arrival);
  ...
  // Average pace from all completed legs
  const avgPace = legPaces.length > 0
    ? legPaces.reduce((a, b) => a + b, 0) / legPaces.length
    : null;

  // Weighted pace (more weight to recent legs)
  ...
  // Predicted ETAs for future locations
  const predictions = derived.map(({ loc, arrival, departure }) => {
    if (arrival || departure) return null; // already have data
  ...
  // Find where we currently are (last location with any logged time)
  ...
  // Decide which rows to show when collapsed
  const visibleDerived = showAll
    ? derived
    : derived.filter((_, i) => {
        // Always show: logged + the next 5 after last logged + FINISH
  ...
  return (
    <div className="app">
      ...
      <main className="main">
          ...
          <table className="table">
            ...
            <tbody>
              {visibleDerived.map(({ loc, arrival, departure, legPace, legHours }, visIdx) => {
                ...
                // Gap row: if we skipped locations, show ellipsis
                ...
                return (
                  <>
                    ...
                    <tr
                      ...
                      style={{ '--cat-color': categoryColor[loc.category], '--cat-bg': categoryBg[loc.category] }}
                    >
                      <td className="col-num">
                        <span className="loc-num">{loc.id}</span>
                      </td>
                      <td className="col-name">
                        <span className="cat-badge" style={{ background: categoryBg[loc.category], color: categoryColor[loc.category] }}>
                          {loc.category === 'Check Point' ? 'CP' : loc.category === 'Paddle Stop' ? 'PS' : 'Ramp'}
                        </span>
                        ...
                        {isEditing ? (
                          <div className="edit-form">
                            <label>
                              ...
                              <input
                                type="datetime-local"
                                value={editValues.arrival}
                                onChange={e => setEditValues(v => ({ ...v, arrival: e.target.value }))}
                              />
                            </label>
                            <label>
                              Departure
                              <input
                                ...
                                onChange={e => setEditValues(v => ({ ...v, departure: e.target.value }))}
                              />
                            </label>
                          ...
                          <button className="btn-edit" onClick={() => openEdit(loc)}>
                            {hasData ? 'Edit' : '+ Log'}
                          </button>
                        )}
                      </td>
                      ...
                      <td className="col-time">
                        {arrival ? (
                          <span className="time-logged">{formatDateTime(arrival)}</span>
                        ) : pred ? (
                          <span className="time-pred">~{formatDateTime(pred)}</span>
                        ) : (
                      ...
                      <td className="col-time">
                        {departure ? (
                          <span className="time-logged">{formatDateTime(departure)}</span>
                        ) : (
                          <span className="time-empty">—</span>
                        )}
                      </td>
                      <td className="col-pace">
                        {legPace != null ? (
                          <span className="pace-val">{legPace.toFixed(1)}<small> mph</small></span>
                        ) : (
                          <span className="time-empty">—</span>
                        )}
                      </td>
        ...
        {!showAll && derived.length !== visibleDerived.length && (
          ...
          <div className="summary-card">
            ...
            <div className="summary-grid">
              <div className="sum-item">
                <span className="sum-label">Weighted Pace</span>
                <span className="sum-val">{(weightedPace || 0).toFixed(2)} mph</span>
              </div>
              <div className="sum-item">
                <span className="sum-label">Average Pace</span>
                <span className="sum-val">{(avgPace || 0).toFixed(2)} mph</span>
              </div>
              <div className="sum-item">
                <span className="sum-label">Miles Done</span>
                <span className="sum-val">{milesCompleted.toFixed(1)}</span>
              </div>
              <div className="sum-item">
                <span className="sum-label">Miles Left</span>
                <span className="sum-val">{(totalMiles - milesCompleted).toFixed(1)}</span>
              </div>
              {predictions[predictions.length - 1] && (
                <div className="sum-item sum-finish">
                  <span className="sum-label">Predicted Finish</span>
                  <span className="sum-val finish-time">{formatDateTime(predictions[predictions.length - 1])}</span>
                </div>
              )}
              {elapsedHrs > 0 && (
                <div className="sum-item">
                  <span className="sum-label">Est. Total Time</span>
                  <span className="sum-val">{formatDuration(elapsedHrs + (totalMiles - milesCompleted) / (paceMph || 1))}</span>
                </div>
              )}
            </div>
          </div>
